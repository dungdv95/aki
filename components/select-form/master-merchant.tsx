...
interface MasterMerchantProps {
  value: string | undefined;
  disable: boolean;
  onChange: (value: string) => void;
}
export default function MasterMerchantSelect({
  value,
  disable,
  onChange,
}: MasterMerchantProps) {
  ...
  const {
    ...
    queryKey: ["/common/get-list-master-merchant"],
    queryFn: () => apis.getCommonMasterMerchant(),
    retry: 0,
  ...
  const list = data?.masterMerchants ?? [];
      ...
      <PopoverTrigger asChild>
        ...
        >
          <span className="truncate">
            {value
              ? list?.find((item: ItemProps) => item.id.toString() === value)
                  ?.name
              : "Lựa chọn Master Merchant..."}
          </span>
          ...
          <CommandGroup className="overflow-scroll overflow-x-hidden">
            {list?.map((item: ItemProps, index: number) => (
              ...
              >
                ...
                />
                <span className="truncate w-[90%]">{item.name}</span>
              </CommandItem>
            ))}
          </CommandGroup>
        </Command>
